...
def test_extent_init(caplog, datatype, hbounds, sbounds, exp_exception, expected_log):
    ...
    with exp_exception as err:

        ext = Extent(label='test', datatype=datatype, 
                     hard_bounds=hbounds, soft_bounds=sbounds)
...
def test_extent_update(caplog, values, expected_log):
    ...
    ext = Extent(label='test', datatype=(int,), 
                 hard_bounds=(0, 120), soft_bounds=(45, 60))
