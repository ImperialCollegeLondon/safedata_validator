...
nodes_file_schema = [('tax_id', 'int PRIMARY KEY'),
                     ('parent_tax_id', 'int'),
                     ('rank', 'text'),
                     ('embl_code', 'text'),
                     ('division_id', 'int'),
...
nodes_output_schema = ', '.join([' '.join(val) for val, drop in zip(nodes_file_schema,
                                nodes_drop_index) if not drop])
nodes_output_schema = f"CREATE TABLE nodes ({nodes_output_schema})"

nodes_insert_placeholders = ','.join(['?'] * (len(nodes_drop_index) - sum(nodes_drop_index)))
nodes_insert_statement  = f"INSERT INTO nodes VALUES ({nodes_insert_placeholders})"

names_output_schema = ', '.join([' '.join(val) for val, drop in zip(names_file_schema,
                                names_drop_index) if not drop])
