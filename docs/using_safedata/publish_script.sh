...
# file called Test_format_good_NCBI.json
safedata_validate Test_format_good_NCBI.xlsx
...
# 2) Upload the dataset files and any external files named in the dataset
#    summary. This uses the zenodo metadata file to confirm the upload destination.
safedata_zenodo upload_file zenodo_1143714.json Test_format_good_NCBI.xlsx

# 3) Update the Zenodo deposit webpage - this populates the deposit description
#    on Zenodo from the dataset metadata
safedata_zenodo upload_metadata zenodo_1143714.json Test_format_good_NCBI.json
...
safedata_server post_metadata zenodo_1143714.json Test_format_good_NCBI.json